...
<%@ page import="com.lec05.board.ReplyVO" %>
...
<form id="boardForm" >
	...
	<tr>
	      <th>작성일</th>
	      <td><input type="text" name="regdate" value="${KEY_BOARDVO.regate}" readonly></td>
	</tr>
	<tr>
	      <th>제목</th>
...
<!-- --------------------------------------- 댓글목록 ------------------------------- -->
<br>
<div id="replyLIstDiv"> <!-- for문 루프 돌릴 필요 없음.  -->
    ...
    <script>
		$(function() {
			...
			$("#replyInsertBtn").click(  function(){
				...
				$.ajax({
					...
					error 	    : function(myval){ console.log("에러:" + myval);   },
			  		success     : function(myval){ 
			  						console.log(myval);
			  						makeTable();  // 건네 줄 값은 없음. 그래서 그냥 함수실행만 하면 화면 댓글 목록 다시 생성됨.			
			  			}
				});	
			});	
			...
			$(document).on("click", ".reply_delete_a", function(event) {
				event.preventDefault();     // href="#" 처리와 동일 == 페이지 이동 막음.
